@extends('layout')

@section('content')

    <section class="hero">
        <article class="hero__tour">
            <p class="section-title">THE ULTIMATE LUXURY</p>
            <h1>Our Offers</h1>
            <div class="navigation">
                <p><a href="../index.html">Home</a> | <span>Offers</span></p>
            </div>
        </article>
    </section>

    <section class="offers">
        <div class="offers__content">
            <article class="offers__item">
                <div class="offers__img">
                    <img src="/assets/images/luxury-room.webp" alt="One of our luxury rooms">
                    <p class="crossed-price"><span>$500</span><span>/Night</span></p>
                    <p class="actual-price"><span>$345</span><span>/Night</span></p>
                </div>
                <div class="offers__description">
                    <p class="section-title">DOUBLE BED</p>
                    <h3 class="start-align">Luxury Double Bed</h3>
                    <p class="section-description">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehend erit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
                    <div class="equipment">
                        <article>
                            <i class="fa-solid fa-wind"></i>
                            <p>Air Conditioner</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-bacon"></i>
                            <p>Breakfast</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-spray-can-sparkles"></i>
                            <p>Cleaning</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-cart-shopping"></i>
                            <p>Groceries</p>
                        </article>
                        <article>
                            <i class="fa-brands fa-shopify"></i>
                            <p>Shops Nearby</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-headset"></i>
                            <p>24/7 Online Support</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-shield-halved"></i>
                            <p>Smart Security</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-wifi"></i>
                            <p>High Speed WI-FI</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-kitchen-set"></i>
                            <p>Kitchen</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-shower"></i>
                            <p>Shower</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-bed"></i>
                            <p>Single Bed</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-toilet-paper"></i>
                            <p>Towels</p>
                        </article>
                    </div>
                    <button class="btn btn--primary">BOOK NOW</button>
                </div>
            </article>
            <article class="offers__item">
                <div class="offers__img">
                    <img src="/assets/images/luxury-room.webp" alt="One of our luxury rooms">
                    <p class="crossed-price"><span>$500</span><span>/Night</span></p>
                    <p class="actual-price"><span>$345</span><span>/Night</span></p>
                </div>
                <div class="offers__description">
                    <p class="section-title">DOUBLE BED</p>
                    <h3 class="start-align">Luxury Double Bed</h3>
                    <p class="section-description">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehend erit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
                    <div class="equipment">
                        <article>
                            <i class="fa-solid fa-wind"></i>
                            <p>Air Conditioner</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-bacon"></i>
                            <p>Breakfast</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-spray-can-sparkles"></i>
                            <p>Cleaning</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-cart-shopping"></i>
                            <p>Groceries</p>
                        </article>
                        <article>
                            <i class="fa-brands fa-shopify"></i>
                            <p>Shops Nearby</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-headset"></i>
                            <p>24/7 Online Support</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-shield-halved"></i>
                            <p>Smart Security</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-wifi"></i>
                            <p>High Speed WI-FI</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-kitchen-set"></i>
                            <p>Kitchen</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-shower"></i>
                            <p>Shower</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-bed"></i>
                            <p>Single Bed</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-toilet-paper"></i>
                            <p>Towels</p>
                        </article>
                    </div>
                    <button class="btn btn--primary">BOOK NOW</button>
                </div>
            </article>
            <article class="offers__item">
                <div class="offers__img">
                    <img src="/assets/images/luxury-room.webp" alt="One of our luxury rooms">
                    <p class="crossed-price"><span>$500</span><span>/Night</span></p>
                    <p class="actual-price"><span>$345</span><span>/Night</span></p>
                </div>
                <div class="offers__description">
                    <p class="section-title">DOUBLE BED</p>
                    <h3 class="start-align">Luxury Double Bed</h3>
                    <p class="section-description">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehend erit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
                    <div class="equipment">
                        <article>
                            <i class="fa-solid fa-wind"></i>
                            <p>Air Conditioner</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-bacon"></i>
                            <p>Breakfast</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-spray-can-sparkles"></i>
                            <p>Cleaning</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-cart-shopping"></i>
                            <p>Groceries</p>
                        </article>
                        <article>
                            <i class="fa-brands fa-shopify"></i>
                            <p>Shops Nearby</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-headset"></i>
                            <p>24/7 Online Support</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-shield-halved"></i>
                            <p>Smart Security</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-wifi"></i>
                            <p>High Speed WI-FI</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-kitchen-set"></i>
                            <p>Kitchen</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-shower"></i>
                            <p>Shower</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-bed"></i>
                            <p>Single Bed</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-toilet-paper"></i>
                            <p>Towels</p>
                        </article>
                    </div>
                    <button class="btn btn--primary">BOOK NOW</button>
                </div>
            </article>
            <article class="offers__item">
                <div class="offers__img">
                    <img src="/assets/images/luxury-room.webp" alt="One of our luxury rooms">
                    <p class="crossed-price"><span>$500</span><span>/Night</span></p>
                    <p class="actual-price"><span>$345</span><span>/Night</span></p>
                </div>
                <div class="offers__description">
                    <p class="section-title">DOUBLE BED</p>
                    <h3 class="start-align">Luxury Double Bed</h3>
                    <p class="section-description">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehend erit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
                    <div class="equipment">
                        <article>
                            <i class="fa-solid fa-wind"></i>
                            <p>Air Conditioner</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-bacon"></i>
                            <p>Breakfast</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-spray-can-sparkles"></i>
                            <p>Cleaning</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-cart-shopping"></i>
                            <p>Groceries</p>
                        </article>
                        <article>
                            <i class="fa-brands fa-shopify"></i>
                            <p>Shops Nearby</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-headset"></i>
                            <p>24/7 Online Support</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-shield-halved"></i>
                            <p>Smart Security</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-wifi"></i>
                            <p>High Speed WI-FI</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-kitchen-set"></i>
                            <p>Kitchen</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-shower"></i>
                            <p>Shower</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-bed"></i>
                            <p>Single Bed</p>
                        </article>
                        <article>
                            <i class="fa-solid fa-toilet-paper"></i>
                            <p>Towels</p>
                        </article>
                    </div>
                    <button class="btn btn--primary">BOOK NOW</button>
                </div>
            </article>
        </div>
    </section>

    <section class="popular-rooms">
        <div class="popular-rooms__content">
            <p class="section-title">POPULAR LIST</p>
            <h3>Popular Rooms</h3>
            <article class="popular-rooms__item">
                <div class="popular-rooms__slider">
                    <img src="/assets/images/duplex-room.webp" alt="One of our duplex rooms">
                    <div class="buttons">
                        <button class="btn btn--primary">&lt;</button>
                        <button class="btn btn--primary">&gt;</button>
                    </div>
                </div>
                <div class="popular-rooms__info">
                    <div class="room-list__icons">
                        <i class="fa-solid fa-bed" style="color: #000000;"></i>
                        <i class="fa-solid fa-wifi" style="color: #000000;"></i>
                        <i class="fa-solid fa-car" style="color: #000000;"></i>
                        <i class="fa-regular fa-snowflake" style="color: #000000;"></i>
                        <i class="fa-solid fa-dumbbell" style="color: #000000;"></i>
                        <i class="fa-solid fa-ban-smoking" style="color: #000000;"></i>
                        <i class="fa-solid fa-martini-glass-citrus" style="color: #000000;"></i>
                    </div>
                    <h3>Minimal Duplex Room</h3>
                    <p class="section-description">Lorem ipsum dolor sit amet, consectetur adipi sicing elit, sed do eiusmod tempor.</p>
                    <div class="room-list__rate">
                        <span>$345/Night</span><span>Booking Now</span>
                    </div>
                </div>
            </article>
        </div>
    </section>
    @endsection